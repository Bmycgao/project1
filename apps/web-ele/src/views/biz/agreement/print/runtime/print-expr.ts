 ...
 * 设计器可在元素 options 中配置：
 * - agreeVisibleWhen: 如 COUNT(rewardItems) > 0、rewardTotal > 0、statusValue == '已签约'
 * - agreeRowFilter: 表格行过滤，如 amount > 0、!EMPTY(name)
 * - agreeRowSort: 表格打印顺序，如 [{ field: 'amount', dir: 'desc' }]
 * - agreeValueExpr: 文本字段计算，如 ROUND(amount, 2)、IF(rewardTotal > 0, rewardTotal, 0)
 */
...
/** 表格打印顺序：一列一条，列头点选会替换为当前列 */
export interface PrintRowSortRule {
  dir: 'asc' | 'desc';
  field: string;
}
...
/**
 * 规范化 agreeRowSort
 * @param raw options.agreeRowSort
 */
export function normalizeAgreeRowSort(raw: unknown): PrintRowSortRule[] {
  if (!Array.isArray(raw)) return [];
  const rules: PrintRowSortRule[] = [];
  for (const item of raw) {
    const field = String((item as PrintRowSortRule)?.field || '').trim();
    const dir = (item as PrintRowSortRule)?.dir === 'desc' ? 'desc' : 'asc';
    if (!field) continue;
...
function isPrintSortEmpty(value: unknown) {
  return value === null || value === undefined || String(value).trim() === '';
}

/**
 * 比较两个单元格值：能当数字就按数字，否则按文本
 * @param left 左值
 * @param right 右值
 */
function comparePrintSortValues(left: unknown, right: unknown) {
...
/**
 * 筛行之后按 agreeRowSort 排打印顺序；空白行始终靠后
 * @param rows 已过滤行
 * @param sort agreeRowSort
 */
export function sortPrintRows<T extends Record<string, unknown>>(
  rows: T[] | undefined,
  sort: unknown,
): T[] {
  const list = rows?.length ? [...rows] : [];
  const rules = normalizeAgreeRowSort(sort);
  if (list.length === 0 || rules.length === 0) return list;
  return list.sort((a, b) => {
    for (const rule of rules) {
      const leftEmpty = isPrintSortEmpty(a[rule.field]);
      const rightEmpty = isPrintSortEmpty(b[rule.field]);
      if (leftEmpty !== rightEmpty) return leftEmpty ? 1 : -1;
      const cmp = comparePrintSortValues(a[rule.field], b[rule.field]);
