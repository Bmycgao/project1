...
describe('print expression DSL', () => {
  ...
  it('counts table rows without a derived hasRewards flag', () => {
    expect(
      evalPrintExpr('COUNT(rewardItems) > 0', {
        rewardItems: [{ name: '签约奖励' }],
      }),
    ).toBe(true);
    expect(evalPrintExpr('COUNT(rewardItems) > 0', { rewardItems: [] })).toBe(
      false,
    );
  });
...
describe('sortPrintRows', () => {
  it('sorts numbers descending and keeps blanks last', () => {
    const rows = [
      { name: 'a', amount: 1 },
      { name: 'b', amount: 3 },
      { name: 'c', amount: '' },
    ...
    expect(
      sortPrintRows(rows, [{ field: 'amount', dir: 'desc' }]).map(
        (row) => row.name,
      ),
    ).toEqual(['b', 'd', 'a', 'c']);
  });
