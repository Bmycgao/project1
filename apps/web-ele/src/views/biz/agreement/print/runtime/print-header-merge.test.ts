...
describe('header-only horizontal merges', () => {
  ...
  it('marks grouped parent cells with leaf range for canvas click-through', () => {
    const projected = projectTableHeaders(columns());
    const group = projected[0]?.find((cell) => cell.title === '项目信息');
    expect(group).toMatchObject({ editorGroupFrom: 1, editorGroupTo: 3 });
  });
  ...
  it('preserves merge metadata in saved template JSON, including empty fields', () => {
