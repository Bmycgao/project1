...
import { prepareFormGrids } from './print-form-grid';
...
function deferCrowdedTableStarts(
  ...
  for (const table of tables) {
    ...
    const flowGroup = isAutoFlowElement(table) ? getFlowGroupName(table) : '';
    const groupMembers = flowGroup
      ? elements.filter(
          (el) => isAutoFlowElement(el) && getFlowGroupName(el) === flowGroup,
        )
      : [table];
    ...
    for (const el of elements) {
      ...
      if (elTop >= anchorTop - 0.1 && (el === table || isAutoFlowElement(el))) {
        el.options.top = elTop + shift;
      }
    }
  }
}
...
export function preparePrintTemplate(
  ...
  const next = isDocumentTemplate(template)
    ? toDocumentTemplate(template)
  ...
  for (const panel of next.panels || []) {
    ...
    if (isDocumentTemplate(next)) {
      ...
      panel.printElements = elements.filter((el: any, index: number) => {
        el.options ||= {};
        el.options.__documentSourceIndex = index;
        const visible =
          !(el.options.agreeFormGrid && el.options.__formRowCount === 0) &&
          evalPrintBool(el.options.agreeVisibleWhen, ctx);
        if (!visible && el.options.agreeFlowCollapse === false) {
          el.options.__documentHidden = true;
          return true;
        }
        return visible;
      });
      compactDocumentHiddenRows(panel, original);
      continue;
    }
    const flowSnaps = snapshotFlowGroups(elements);
    panel.printElements = elements.filter(
      (el: any) =>
        !(el?.options?.agreeFormGrid && el.options.__formRowCount === 0) &&
        evalPrintBool(el?.options?.agreeVisibleWhen, ctx),
    );
    compactPanelFlow(panel, flowSnaps, elements);
