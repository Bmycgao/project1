...
import { buildPresetTableColumns } from '../data/fields';
import { createFormGrid } from '../runtime/print-form-grid';
import { normalizeTemplateWatermark } from '../runtime/print-watermark';
...
/**
 * 规范单元格水平对齐；未写 align 时按居中（与内置模板一致）
...
function newLeafCell(patch: Partial<LeafTableCell>): Record<string, any> {
  const cell: Record<string, any> = {
    ...
    align: patch.align ? normalizePrintAlign(patch.align) : 'center',
    colspan: 1,
    rowspan: 1,
    checked: true,
  };
...
> = {
  ...
  table: { tid: 'agreePrintModule.table', title: '明细表', type: 'table' },
  formGrid: { tid: 'agreePrintModule.table', title: '自由表单', type: 'table' },
...
export function buildToolboxPrintElement(
  ...
  if (type === 'formGrid') {
    ...
    return {
      ...
      printElementType: typeMeta('formGrid'),
    };
  }
  if (type === 'table') {
    return {
      options: {
        ...
        columns: normalizeTableColumns([
          [
            { title: '列1', field: 'col1', width: 180, align: 'center' },
            { title: '列2', field: 'col2', width: 180, align: 'center' },
            { title: '列3', field: 'col3', width: 190, align: 'center' },
          ],
        ]),
      },
